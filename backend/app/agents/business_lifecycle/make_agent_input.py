...
def get_score_missing_reason(
    ...
    if (
        pd.notna(observed_quarters)
        and int(observed_quarters) < quarter_count
    ):
        return (
...
def build_agent_input(
    ...
    for _, row in scored_df.iterrows():
        industry = {
            ...
            "metrics": {
                # 데이터 확보 정도
                "observed_quarters": to_int(
                    row["observed_quarters"]
                ),

                # 현재 규모
                "latest_store_count": to_int(
                    row["latest_store_count"]
                ),
                "avg_store_count": to_float(
                    row["avg_store_count"]
                ),

                # 전체 분석기간
                "period_open_count": to_int(
                    row["period_open_count"]
                ),
                "period_close_count": to_int(
                    row["period_close_count"]
                ),
                "period_net_change": to_int(
                ...
                # 최근 1년
                "recent_year_open_count": to_int(
                    row["recent_year_open_count"]
                ),
                "recent_year_close_count": to_int(
                    row["recent_year_close_count"]
                ),
                "recent_year_net_change": to_int(
                    row["recent_year_net_change"]
                ),

                "recent_year_close_rate": to_float(
                    row["recent_year_close_rate"]
                ),
                "recent_year_net_change_rate": to_float(
                    row["recent_year_net_change_rate"]
                ),

                # 과거와 최근의 비교
                "oldest_year_close_rate": to_float(
                    row["oldest_year_close_rate"]
                ),
                "close_rate_trend": to_float(
                    row["close_rate_trend"]
                ),
            },
            ...
            "component_scores": {
                "recent_close_rate_score": to_float(
                    row["recent_close_rate_score"]
                ),
                ...
                "close_trend_score": to_float(
                    row["close_trend_score"]
                ),
            },
    ...
    agent_input: dict[str, Any] = {
        ...
        "scoring_method": {
            ...
            "notes": [
                ...
                (
                    "close_rate_trend가 음수이면 "
                    "과거보다 최근 폐업률이 낮아진 것이다."
                ),
                (
                    "close_rate_trend가 양수이면 "
                    "과거보다 최근 폐업률이 높아진 것이다."
                ),
            ],
        },
        ...
        "coverage": {
            ...
            "scored_industries": len(
                scored_df
            ),
            "unscored_industries": len(
                unscored_df
            ),
        },
