...
class AgentContractTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_accepts_plain_dict_task(self):
        result = await analyze(
    ...
    async def test_no_data_when_no_stores(self):
        result = await analyze(
            self.task, settings=self.settings, store_client=FakeClient(self.settings, [])
        )
    ...
    async def test_district_baseline_is_used_when_code_present(self):
    ...
    async def test_district_failure_is_not_fatal(self):
        stores = sample_stores(district_code="11710", district_name="송파구")
        client = FakeClient(
...
class ClientBehaviourTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _client(self, payload):
        ...
        return StoreClient(self.settings, client=httpx.AsyncClient(transport=transport))

    async def test_nodata_returns_empty_instead_of_raising(self):
        stores, meta = await self._client(NODATA_PAYLOAD).stores_in_radius(37.5, 127.0, 10)
        self.assertEqual(stores, [])
        self.assertEqual(meta["total_count"], 0)

    async def test_agent_reports_no_data_for_empty_area(self):
        task = AnalysisTask.model_validate({"request_id": "request-002", "site": SITE})
        result = await analyze(
            task, settings=self.settings, store_client=self._client(NODATA_PAYLOAD)
        )
        self.assertEqual(result.status, "no_data")
        self.assertIsNone(result.error)

    async def test_district_second_call_uses_cache(self):
        items = [
            {
                "bizesId": f"S{i}",
                "bizesNm": f"가게{i}",
        ...
        first, _ = await client.stores_in_district("11710")
        after_first = calls["n"]
        second, meta = await client.stores_in_district("11710")
    ...
    async def test_agent_reports_error_on_auth_failure(self):
        task = AnalysisTask.model_validate({"request_id": "request-003", "site": SITE})
        result = await analyze(
            task, settings=self.settings, store_client=self._client(AUTH_PAYLOAD)
        )
        self.assertEqual(result.status, "error")
